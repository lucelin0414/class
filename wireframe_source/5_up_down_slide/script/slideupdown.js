...
function start(){    //start 함수 생성
    //.slide_box > img의 첫번째 요소의 형제들에게 .style(css) 속성 margin-top:-400px를 추가함
...
function slide(){
    ...
    //now-1번째 <img>태그의 style(css) 속성에 margin-top:-400px을 추가하고, 이미지를 사라지게 함.
    $(".slide_box > img").eq(now-1).css({"margin-top":"-400px"});


    //now번째 <img>태그의 style(css)속성에 margin-top:0px을 추가하고, 이미지를 불러오는 구문
