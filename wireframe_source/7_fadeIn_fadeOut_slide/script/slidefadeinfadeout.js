...
function start(){    //start 함수 생성
    //.slide_box > img의 첫번째 요소의 형제들에게 fadeOut()함수로 이미지를 사라지게 함.
...
function slide(){
    ...
    //now-1번째 <img>태그의 style(css) 속성에 fadeOut() 함수로 이미지를 사라지게 함.
    $(".slide_box > img").eq(now-1).fadeOut('slow')


    //now번째 <img>태그의 style(css)속성에 faedIn()함수로, 이미지를 불러오는 구문
